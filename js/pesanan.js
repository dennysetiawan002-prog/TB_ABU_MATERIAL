...
async function loadDaftarPesanan() {
  ...
  data.forEach(p => {
    ...
    tr.innerHTML = `
      ...
      <td>${p.status || "-"}</td>
      <td>
        <button class="btn-aksi btn-detail">Rincian</button>
        <button class="btn-aksi btn-edit">Edit</button>
      </td>
    `;

    // 🔗 EVENT RINCIAN (AMAN)
    tr.querySelector(".btn-detail")
      .addEventListener("click", () => {
        bukaDetailPesanan(p.no_pesanan);
      });

    // 🔗 EVENT EDIT (AMAN)
    tr.querySelector(".btn-edit")
      .addEventListener("click", () => {
        editPesanan(p.no_pesanan);
      });
...
/*************************
 * RINCIAN PESANAN (STUB AMAN)
 *************************/
async function bukaDetailPesanan(noPesanan) {
  try {
    const res = await apiGet("getPesananDetail&no=" + noPesanan);
    ...
    let html = `
      ...
      <p><b>Nama Toko:</b> ${res.nama_toko}</p>
      <p><b>Tanggal:</b> ${new Date(res.tanggal).toLocaleString()}</p>
      <hr>
    ...
    html += `
      ...
      <p style="text-align:right">
        <b>Total: Rp ${Number(res.total).toLocaleString()}</b>
      </p>
    `;
    ...
    console.error(err);
    alert("Gagal memuat rincian pesanan");
  }
}
...
window.bukaDetailPesanan = bukaDetailPesanan;
